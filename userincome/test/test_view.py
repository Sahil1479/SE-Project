...
# class TestUserIncomeView(TestCase):
#     def setUp(self):
#         self.c = Client()
#         self.response = self.c.login(username='john',
#             password='123456')
#     def test_unauthenticated_user(self):
#         response = self.c.get('http://127.0.0.1:8000/income/')
#         self.assertEqual(response.status_code, 302)
#     def test_authenticated_user(self):
#         self.response = self.client.get(reversed('userincome:income'))
...
class TestUserIncome(TestCase):
    def setUp(self):
        ...
        # self.edit_income_url = reverse('income-edit')
        # self.delete_income_url = reverse('income-delete')
    ...
    # def test_should_render_income_correctly(self):
    #     self.client.login(username='sahil', password='password123')
    #     self.create_income()
    #     created_income = UserIncome.objects.filter(owner=self.user).order_by('-id')
    #     # print(created_income[0])
    #     # print(created_income)
    #     res = self.client.get(reverse('income',
    #                                   kwargs={'id': int(created_income.id)}),
    #                           format='json')
    #     self.assertEqual(res.data['id'], created_income.id)
